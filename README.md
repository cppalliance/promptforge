...
`cargo build -p workshop` is a low-level package build. It requires a real gateway executable to have already been staged at `crates/workshop/desktop/binaries/promptforge-gateway-<target-triple>` and does not clean that staging afterward. Bundling with `cargo tauri build` has the same staging requirement; the release workflows under `.github/workflows/` show the exact packaging commands per platform.
