...
- `crates/promptforge-core-tests` - unpublished binary and test harness: owns complete author-shaped valid, invalid, and deterministic execution prompt documents, registers each fixture explicitly with `include_str!`, and asserts parsed structure, stable public error contracts, exact Lua checkpoints, scalar preamble return, store fall-through, and concurrent execution-ID separation through deterministic no-tools declaration binding and the public parsed-`Prompt` execution seam. It also discovers every shipped repository prompt, rejects concrete tool names, and requires each to parse; shipped semantic binding remains covered by the MCP server's owner tests. Its dormant real-model provisioner pins official CPU-only llama.cpp b10082 archives and Qwen3-0.6B Q8 weights by URL and SHA-256 under `.model-cache/`. Ordinary tests exercise downloads, verification, extraction, atomic installation, cache reuse, and repair only through local fake HTTP servers and tiny archives. They never contact those official hosts, launch llama-server, or run a model. The binary entry point remains reserved for the explicit real-model runner
...
## Real-model artifact cache

The unpublished core-tests crate contains the provisioner for a later explicit real-model runner. It targets Windows and Linux on x86-64 or arm64, plus macOS on x86-64 or Apple Silicon. The llama.cpp sidecar is pinned to official CPU-only release b10082 archives, and the model is pinned to official `Qwen/Qwen3-0.6B-GGUF` file `Qwen3-0.6B-Q8_0.gguf` with SHA-256 `9465e63a22add5354d9bb4b99e90117043c7124007664907259bd16d043bb031`.
