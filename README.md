...
- `crates/promptforge-core-tests` - unpublished binary and offline fixture harness: owns complete author-shaped valid, invalid, and deterministic execution prompt documents, registers each fixture explicitly with `include_str!`, and asserts parsed structure, stable public error contracts, exact Lua checkpoints, scalar preamble return, store fall-through, and concurrent execution-ID separation through deterministic no-tools declaration binding and the public parsed-`Prompt` execution seam. It also discovers every shipped repository prompt, rejects concrete tool names, and requires each to parse; shipped semantic binding remains covered by the MCP server's owner tests. The harness directly depends only on `promptforge-core` and performs no picker construction, model loading, tool provisioning, process launches, network requests, gateway calls, or generation-model calls; a cold workspace build may still acquire assets for core's existing transitive picker dependency. The binary entry point is reserved for the explicit real-model runner
