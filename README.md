...
- `crates/promptforge-core-tests` - unpublished binary and test harness: owns complete author-shaped valid, invalid, deterministic offline, real-text, and real-tool-call prompt documents. Ordinary tests assert public parse and error contracts, exact Lua checkpoints, scalar preamble return, store fall-through, concurrent execution-ID separation, and artifact handling without external access. `cargo run -p promptforge-core-tests` (or `... scenarios`) starts a temporary `promptforge-gateway` with a pinned Qwen3-0.6B profile (gateway owns GGUF/llama-server under `~/.promptforge/`), connects `GatewayClient` to that gateway, and behaviorally verifies text, one-string aliased tool dispatch, tool-result continuation, final answer, epilog, and turn budgets. Interactive prompt development is not this crate - use `promptforge-dev`
- `crates/promptforge-dev` - unpublished binary for interactive prompt development against an already-running `promptforge-gateway`. Runs one prompt file with optional input and `--watch`; never starts the gateway or `llama-server`. Author workflow: [`crates/promptforge-dev/README.md`](crates/promptforge-dev/README.md)
...
## Explicit real-model tests

The unpublished core-tests crate targets Windows and Linux on x86-64 or arm64, plus macOS on x86-64 or Apple Silicon. Ordinary `cargo test -p promptforge-core-tests` stays fully offline. The opt-in scenario suite runs through a temporary `promptforge-gateway` whose profile pins official `Qwen/Qwen3-0.6B-GGUF` file `Qwen3-0.6B-Q8_0.gguf` (SHA-256 `9465e63a22add5354d9bb4b99e90117043c7124007664907259bd16d043bb031`); the gateway owns download/caching under `~/.promptforge/` and spawns `llama-server`. Build the gateway first, then from the repository root:
...
Both spellings run the same fixed scenario suite. The harness writes a temporary gateway profile, starts `promptforge-gateway serve` on a free loopback port with a random bearer token, waits until `GET /health` and authenticated `GET /v1/models` advertise the local model, then runs the text and tool-call fixtures through `GatewayClient`. Dropping the guard kills the gateway process tree. Full pin provenance, readiness, and fixture contracts live in [`crates/promptforge-core-tests/README.md`](crates/promptforge-core-tests/README.md).

## The prompt dev loop

Edit a prompt, run it against an already-running gateway, read the result. Interactive development lives in `promptforge-dev`, not in `promptforge-core-tests`. Start `promptforge-gateway` yourself, export `PROMPTFORGE_GATEWAY_URL` and `PROMPTFORGE_GATEWAY_KEY`, then:
...
models.always("analyst", "A model suited for careful analysis", { thinking = false, temperature = 0, context = 40000 })
...
- Shared Lua declares semantic capabilities under prompt-local aliases: `tools.need(alias, description)` for tools and `models.need(alias, description, opts?)` for models. A section exposes a tool alias with `tools.add(alias)` and selects a model binding with `models.use(alias)` (at most once). `tools.always(alias)` belongs in shared Lua only when every model-facing section genuinely needs that capability. `models.always(alias)` sets a prompt-wide default model binding for sections that omit `models.use`. `models.always` also accepts the combined form `models.always(alias, description, opts?)`, which declares and selects in one call (sugar for `models.need` followed by `models.always`). At most one `models.always` call is permitted per prompt regardless of form. Declaration alone exposes nothing and selects no model; non-empty model-facing prose without `models.use` or `models.always` fails with `Error::ModelRequired`. Whitespace-only prose skips the model. After scope close, `tools.calls["alias"]` is the per-section count of model dispatches for that in-scope alias (including failed tool results); indexing an out-of-scope key is a hard error. Epilogs use ordinary Lua `assert(tools.calls["search"] > 0)`. Standing rule: no defaults, everything explicit - see `crates/promptforge-core/design-core.md`.
...
- `debug` is the opt-in [`DebugCapture`](promptforge_core::debug::DebugCapture) seam for raw model request and response bodies, including optional `finish_reason` and `reasoning_content` side channels. Reasoning is never treated as the answer text; empty product still fails even when reasoning is present. Production hosts leave `debug` as `None`. The dev runner wires a capture that writes each turn under `<prompt-stem>.store/.trace/`.
