...
- `crates/promptforge-core-tests` - unpublished binary and offline fixture harness: owns complete author-shaped valid and invalid prompt documents, registers each one explicitly with `include_str!`, and asserts parsed structure or stable public error contracts through the `promptforge-core` API. Its ordinary tests perform no downloads, network calls, model calls, or process launches; the binary entry point is reserved for the explicit real-model runner
