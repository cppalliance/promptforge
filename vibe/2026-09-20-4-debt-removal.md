...
todos:
  ...
  - id: walled-tier-inversion
    content: Split admin/walled/handoff.rs at its cfg seam, rehome SpeechSnapshot, ShutdownSignal, and config_write_error, and repoint the import sites
    status: pending
  - id: walled-tier-ratchet
    content: Add a build-xtask rule forbidding admin::walled paths outside the tier, allowlisting the assembly sites
    status: pending
  - id: error-source-unification
    content: Collapse the nine error-source newtypes onto three shared types with accessors and record the new component in the architecture record
    status: pending
isProject: false
---
...
- Success criteria:
  ...
  - A crate-wide search for `admin::walled::` outside `crates/gateway/app/src/admin/walled/` returns hits only in `crates/gateway/app/src/lib.rs` and `crates/gateway/app/src/registry.rs`, and the new check fails when it returns any other file.
- Constraints:
...
- Modules and interfaces:
  ...
  - `crates/build-xtask/src/tidy.rs` gains a boundary rule: no file outside `crates/gateway/app/src/admin/walled/` may name a `crate::admin::walled::` path, with an allowlist of two files, `crates/gateway/app/src/lib.rs` for the `AppState` field types and the router merge, and `crates/gateway/app/src/registry.rs` for the route enumeration. The allowlist is keyed on file paths, never on line numbers.
  ...
  - `SpeechSnapshot` moves out of `admin/walled/system.rs`, `ShutdownSignal` out of `admin/walled/shutdown.rs`, and `config_write_error` out of `admin/walled/config.rs` into `error.rs`, following the precedent already set when `error_chain` moved there.
...
- Exit criteria:
  ...
  - A crate-wide search for `admin::walled::` outside the tier returns only the three assembly sites, and the new boundary rule enforces that result.
...
- Decisions:
  ...
  - Sequence the work as one step per work item, six in total, and give every step the same single component name, so no step but the last sits at a component boundary. This is deliberate and governs: the areas named in the technical design are regions of the codebase, not units of delivery, and splitting them into separate components would force a wide verification pass at four internal boundaries where a focused one suffices. Carry no separate step for verification. The walled-tier split and the rehoming of `SpeechSnapshot`, `ShutdownSignal`, and `config_write_error` are one relocation with one exit condition, so they are one step. Creating the shared error-source crate and collapsing the nine newtypes onto it are one change, because a crate created without its consumers is dormant code. The wide gates belong to the last step rather than to a step of their own, since a step that only runs gates produces a commit with no code and the final step's verification already runs them.
...
- Assumptions, risks, and notes:
  ...
  - The state and router assembly sites are accepted and stay: `lib.rs:132` holds walled module types as `AppState` fields, `lib.rs:486` merges the walled routes, `registry.rs:77` enumerates them, and `test_support.rs` builds a fixture proxy. These are where the tier's modules are legitimately named.
  ...
  - Risk: the boundary rule's allowlist is the fiddly part. Too narrow and legitimate assembly fails; too wide and the rule stops catching anything. The three named sites are the whole allowlist.
...
### Step 4: Walled-tier relocation - the handoff split and the three rehomed items
...
- Move `SpeechSnapshot` out of `admin/walled/system.rs`, `ShutdownSignal` out of `admin/walled/shutdown.rs`, and `config_write_error` out of `admin/walled/config.rs` into `crates/gateway/app/src/error.rs`, following the precedent set when `error_chain` moved there. All three stay `pub(crate)`.
- Repoint every site naming one of the three: `admin/open/status.rs`, `admin/open/progress.rs`, `admin/open/progress-tests.rs`, `admin/open/profiles.rs`, `commands-apply.rs`, `runner.rs`, `test_support.rs:58,77,91`, and the `AppState` fields at `lib.rs:132`, leaving in place whatever walled types `lib.rs:132` still legitimately holds.
- After this step, the only files outside `crates/gateway/app/src/admin/walled/` naming a `crate::admin::walled::` path are `crates/gateway/app/src/lib.rs`, for the `AppState` field types and the router merge, and `crates/gateway/app/src/registry.rs`, for the route enumeration. Confirm by crate-wide search before committing, because step 5's allowlist is exactly those two files. Identify the sites by file and role, never by line number, since this step moves the lines.
...
- Component: debt-removal
- Add a rule to `crates/build-xtask/src/tidy.rs`: no file outside `crates/gateway/app/src/admin/walled/` may name a `crate::admin::walled::` path. The allowlist is exactly two files, `crates/gateway/app/src/lib.rs` for the `AppState` field types and the router merge, and `crates/gateway/app/src/registry.rs` for the route enumeration. Key the allowlist on file paths, never on line numbers, which move with any edit. Too narrow and legitimate assembly fails; too wide and the rule catches nothing.
- The rule protects the directory's meaning, not the wall. Wall enforcement stays structural at the router merge in `lib.rs:485-488` and does not move.
- Tests in `crates/build-xtask`: the rule fires, naming the file, for a non-allowlisted file outside the tier that names such a path, and stays silent for each of the three allowlisted sites.
...
### Step 6: shared-error-source crate and the nine newtypes collapsed onto it
...
- Add the crate to the component list in `vibe/archdoc.md` under shared substrate. Creating the crate and collapsing the newtypes onto it are one commit by decision, because a crate created without its consumers is dormant code.
- Remove `JsonSource` at `crates/gateway-api-discovery/src/error.rs:13`, `crates/gateway/local/src/error.rs:12`, `crates/workshop/user-state/src/error.rs:70`, and `crates/workshop/workspace/src/error.rs:190`; `HttpSource` at `crates/gateway/local/src/error.rs:25` and `crates/gateway/cloud-providers/src/lib.rs:133`; and `DatabaseSource` at `crates/harness/log/src/error.rs:51,58` and `crates/workshop/workspace/src/workspace_file.rs:122`. Fold `PayloadSource`, which wraps `serde_json::Error`, onto the shared JSON type.
- Repoint `LogError`, `WorkspaceFileError`, `WorkspaceError`, `UserStateError`, `DiscoveryError`, `LocalError`, and `FetchError`, and add `shared-error-source` with only the needed feature to each of the six consuming crates. This breaks those public variant shapes, all within this workspace.
...
- Tests: for each repointed enum, a case asserting the wrapped third-party error is reachable through the shared type, either by accessor or by walking the chain.
- Verify this step's own change with focused runs: `cargo nextest run --locked -p shared-error-source --all-features`, `cargo nextest run --locked -p harness-log -p gateway-api-discovery -p gateway-local -p gateway-cloud-providers --all-features`, the workshop partition `cargo nextest run --locked -p workshop-workspace -p workshop-user-state`, and `cargo test -p build-xtask`, which checks the tier graph, container privacy, and lint inheritance for the new crate.
...
- Confirm a crate-wide search for `admin::walled::` outside `crates/gateway/app/src/admin/walled/` returns hits only in `crates/gateway/app/src/lib.rs` and `crates/gateway/app/src/registry.rs`, and that the step 5 rule fails when it returns any other file.
