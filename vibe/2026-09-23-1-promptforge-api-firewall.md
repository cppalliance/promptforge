...
- States and validation:
  ...
  - Closed: every item the surface mentions is a facade re-export, comes from std/core/alloc, or comes from an allowlisted crate (`serde`, `serde_json`).
...
- Decisions:
  ...
  - **Third-party allowlist for the public surface: std/core/alloc, `serde`, `serde_json`.** Adding a crate is an explicit, reviewed change to one constant. The allowlist restricts what the surface mentions, not what internal crates depend on. The user: "I don't mind depending on serde or whatever. basic rust shit is ok.", and "promptforge internal crates can use a lot of external crates. but the promptforge-api would be restricted to some basic ones".
...
### Step 7: Implement cargo xtask api
...
- `crates/build-xtask/src/api.rs` is the subcommand entry for `cargo xtask api`, with `--check` and `--bless`, dispatched from `main.rs` beside `new-crate` and `tidy`. Its submodules live in `crates/build-xtask/src/api/`, per the flat-directory rule for three or more submodules:
  ...
  - `closure.rs`: walks the surface, meaning signatures, fields, variants, generic bounds, supertraits, both sides of every trait impl, associated types, and each surface item's `links` table. Every mention must be a facade re-export, std, core, or alloc, or come from a crate in one allowlist constant (`serde`, `serde_json`).
