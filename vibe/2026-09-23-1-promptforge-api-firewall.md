...
- Non-goals:
  ...
  - Changing engine behavior or the harness and workshop architecture. Run-log and replay formats may change where a step needs it, because no recorded logs exist yet.
...
- Data, persistence, failure, security, and privacy constraints:
  - Wire JSON formats don't change. Run-log and replay formats may change where a step needs it, for example to report diagnostics that would otherwise be dropped, because no recorded logs exist yet; JSON that reaches them still round-trips exactly. Serde derives on public types remain part of the surface, as they are today.
...
- Regression, security, and performance:
  ...
  - Run-log JSON round-trip tests keep passing, and any new run-log event gets round-trip coverage.
- Exit criteria:
...
- Decisions:
  ...
  - **Run-log and replay formats may change.** The original non-goal kept them fixed, but no recorded logs exist yet, so a step may change them where it needs to, such as reporting the model client's malformed-metadata diagnostics through the engine instead of dropping them. Wire JSON formats stay fixed. The user, on 2026-09-24: "I dont care about changing the run-log format no one has any recorded logs yet".
  - **The surface listing is compact about impls.** `public-api.txt` leaves out impls of `core::marker::StructuralPartialEq` and the nightly-only `core::clone::TrivialClone` and `core::marker::UnsafeUnpin`, which no host can depend on and which would churn whenever the pinned nightly moves. Auto traits collapse to one line per type, `auto <path>: <traits>`, naming `Freeze`, `RefUnwindSafe`, `Send`, `Sync`, `Unpin`, and `UnwindSafe` in alphabetical order with `!` before each one the type lacks. Derive-style impls (`Clone`, `Copy`, `Debug`, `Default`, `Eq`, `Hash`, `Ord`, `PartialEq`, `PartialOrd`, `Serialize`, `Deserialize`) collapse to one line per type, `derives <path>: <traits>`, in alphabetical order. An impl whose header bounds a type parameter keeps its own line, and every other impl (`From`, `Display`, `Error`, the VFS traits, and so on) stays one line each. A type losing `Send` or gaining `Serialize` still changes its line. Shown a 2,782-line listing (about 1,160 lines of API, 756 of auto traits, 550 of derive-style impls, 120 of markers), the user chose on 2026-09-24 to drop the markers and collapse both groups, for about 1,600 lines.
  - **The VFS mode gate joins `vfs`.** `Mode`, `ModePolicy`, and `ModeHandle` are re-exported in `promptforge::vfs`: `ModePolicy` implements the open `Policy` trait, so closure requires it, and hosts may gate their own mounts by mode. `STORE_MOUNT` and `empty` stay internal. Offered deleting the gate, which nothing wires up yet, the user chose on 2026-09-24 to keep the re-exports.
...
### Step 11: Bless the surface listing and gate it in CI [completed]
...
- Sweep READMEs, crate `AGENTS.md` files, `guide/`, `tools/document.md`, `clippy.toml` comments, and comments in source files outside the engine (known: `crates/harness-api/src/lib.rs` and `crates/gateway/app/src/dialect.rs`) for `promptforge-api-runtime`, `promptforge_api_runtime`, `promptforge-api-types`, `promptforge_api_types`, `shared-vfs`, and `shared_vfs`, plus container paths `crates/promptforge/<engine crate>` left stale by Step 1. Update or reword each hit. Also correct `vibe/archdoc.md`, which still says the store is exposed as `vfs.store(&access)`; Step 11 removed `StoreExt`, and the engine builds a store with `Store::new(&access)`. Leave the Lua chunk-name strings `@crates/promptforge/lua/...` (lua `coro.rs` lines 31 and 39, `messages.rs` line 28) unchanged: they are labels, not file paths, and tests or recorded error text may depend on them.
