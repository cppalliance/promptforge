...
- workshop UI: desktop authoring shell and in-process server that drive agent sessions through `harness` and attach over the gateway protocol; persists user-scoped UI state through `workshop-user-state` (one JSON file in the state directory) and workspace-scoped UI state through the `.pfwork` workspace file; depends on: harness, gateway, store, shared substrate
