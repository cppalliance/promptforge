...
- The crate never names gateway concepts (`GatewayError`, `AppState`, `check_auth`); failures return its own `WebSearchError`, which wraps `ProtocolError` from `gateway-protocol`.
