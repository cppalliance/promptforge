//! Verification of the prebuilt workshop UI artifact (`ui/dist/` plus its
...
/// `ui/build.mjs`.
pub(crate) const STATIC_FILES: &[&str] = &[
    "index.html",
    "style.css",
    "pcm-worklet.js",
...
/// The dist-relative names `routes::assets` serves; a packaged artifact
...
pub(crate) fn verify(ui_dir: &Path) -> Result<(), String> {
    verify_inner(ui_dir).map_err(|reason| {
        format!(
            "the workshop UI artifact at ui/dist/ cannot be embedded: {reason}\n\n{INSTRUCTIONS}"
        )
    })
}
...
mod tests {
    ...
    fn wrong_manifest_version_fails_verification() {
        ...
        .expect("rewrite manifest");
        let error = verify(&ui_dir).expect_err("a foreign manifest version must fail");
    ...
    fn missing_served_file_fails_verification() {
        let (_temp, ui_dir) = fixture_ui();
        fs::remove_file(ui_dir.join("dist/app.css")).expect("remove served file");
        let error = verify(&ui_dir).expect_err("a missing served file must fail");
        assert!(error.contains("app.css"), "{error}");
    }
    ...
    #[test]
    fn malformed_manifest_fails_verification() {
        let (_temp, ui_dir) = fixture_ui();
        fs::write(ui_dir.join("dist/manifest.json"), "{ not json\n").expect("rewrite manifest");
        let error = verify(&ui_dir).expect_err("malformed JSON must fail");
        assert!(error.contains("not valid JSON"), "{error}");
    }

    #[test]
    fn manifest_without_input_hash_fails_verification() {
        let (_temp, ui_dir) = fixture_ui();
        let manifest = format!(
            "{{\n  \"version\": {MANIFEST_VERSION},\n  \"minified\": true,\n  \"files\": {REQUIRED_SERVED:?}\n}}\n",
        );
        fs::write(ui_dir.join("dist/manifest.json"), manifest).expect("rewrite manifest");
        let error = verify(&ui_dir).expect_err("a missing inputHash must fail");
        assert!(error.contains("no inputHash"), "{error}");
    }

    #[test]
    fn manifest_without_files_list_fails_verification() {
        let (_temp, ui_dir) = fixture_ui();
        let manifest = format!(
            "{{\n  \"version\": {},\n  \"minified\": true,\n  \"inputHash\": \"{}\"\n}}\n",
            MANIFEST_VERSION,
        ...
        fs::write(ui_dir.join("dist/manifest.json"), manifest).expect("rewrite manifest");
        let error = verify(&ui_dir).expect_err("a missing files list must fail");
    ...
    fn escaping_manifest_entry_fails_verification() {
        ...
        fs::write(
            ui_dir.join("dist/manifest.json"),
            text.replace("\"app.css\",", "\"app.css\", \"../escape.txt\","),
        )
        .expect("rewrite manifest");
