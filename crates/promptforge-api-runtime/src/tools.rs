...
//!
//! The engine never holds a tool implementation: it fills its slots by
//! identity against the host-supplied [`ToolCatalog`] of descriptors and
//! issues each call as a `ToolCall` effect naming the [`ToolId`], which the
//! host resolves against its own implementations (the harness's `Tool`
//! trait, in `harness-capabilities`). The runtime-agnostic vocabulary -
