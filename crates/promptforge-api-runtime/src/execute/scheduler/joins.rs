...
struct ArmTemplate<'a> {
    ...
    /// The fanout caller's task: an arm reports it as its `sys.taskid`,
    /// since the arms are the caller's own work, not tasks of their own.
    task: TaskId,
...
impl<'a> Scheduler<'a> {
    ...
    fn refill_fanout(&mut self, fanout: FanoutId) -> Result<()> {
        loop {
            ...
            // The arm is the caller's own work: it reports the caller's task.
            self.chains[chain.index()].task.clone_from(&template.task);
