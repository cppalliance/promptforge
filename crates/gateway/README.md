...
A profile is a checklist over the local and STT catalogs: checked local models spawn `llama-server` children, checked STT models select the gateway-owned transcription engine. Every `[[model]]` routes regardless of profile, and a profile that names a remote model fails validation. Membership is the entire definition; profiles carry no per-field overrides. The profile is read once at boot, and the local model set it selects is fixed for the process lifetime.
