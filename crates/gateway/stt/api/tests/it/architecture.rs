//! Cargo metadata checks for the five approved product dependency
//! boundaries. The Workshop-to-Gateway arm exempts the gateway family's
//! public pair (`gateway-api`, `gateway-api-discovery`): the workshop
//! attaches to a running gateway through exactly those two crates.
...
struct DependencyRule {
    ...
    /// Forbidden-set packages a dependent may still name.
...
const PRODUCT_DEPENDENCY_RULES: [DependencyRule; 5] = [
    ...
    DependencyRule {
        ...
        allowed: &["gateway-api", "gateway-api-discovery"],
...
fn workspace_root() -> PathBuf {
    // Walk ancestors instead of counting parents: the crate moves between
    // container depths, and depth counting has broken on every past move.
    ...
    panic!("no ancestor of CARGO_MANIFEST_DIR carries a workspace manifest");
}
                    ...
                    PRODUCT_DEPENDENCY_RULES
                        ...
                        .filter(move |rule| {
                            rule.dependent.contains(&package.name)
                                ...
                                && !rule.allowed.contains(&dependency)
                        })
                        .map(move |rule| {
                            format!(
