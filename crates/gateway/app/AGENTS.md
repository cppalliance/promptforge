...
- A boot config with a legacy `[workshop]` section must keep parsing. Startup warns that its `bind` and `open_browser` fields are inert instead of failing or silently ignoring them.
