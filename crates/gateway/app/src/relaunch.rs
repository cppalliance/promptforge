...
pub enum GatewayStartup {
    ...
    Boot(gateway_api_discovery::GatewayInstanceLease),
...
pub enum GatewayStartupError {
    ...
    Lease(#[source] gateway_api_discovery::SidecarError),
    ...
    Resolve(#[source] gateway_api_discovery::SidecarError),
...
fn settings_url(connection: &gateway_api_discovery::ValidatedConnection) -> String {
    crate::auth::primitives::auth_url(
...
pub fn settle_gateway_startup(
    ...
    match gateway_api_discovery::GatewayInstanceLease::try_acquire(&run_dir)
        .map_err(GatewayStartupError::Lease)?
    {
        Some(lease) => {
            let resolution =
                gateway_api_discovery::resolve(&run_dir).map_err(GatewayStartupError::Resolve)?;
...
fn wait_for_owner(
    ...
    wait_for_owner_with(timeout, |deadline| {
        let connection = gateway_api_discovery::GatewayDiscoveryFile::read(run_dir).ok()??;
        let validated =
            gateway_api_discovery::ValidatedConnection::validate_before(connection, deadline)
                .ok()?;
...
mod tests {
    ...
    fn live_file() -> gateway_api_discovery::GatewayDiscoveryFile {
    ...
    fn a_live_file_hands_off_its_settings_url() {
        let decision = decide(&gateway_api_discovery::Resolution::Attach(live_file()));
        assert_eq!(
            decision,
    ...
    fn a_live_file_with_a_query_special_key_encodes_the_url() {
        ...
        let decision = decide(&gateway_api_discovery::Resolution::Attach(file));
        assert_eq!(
            decision,
    ...
    fn absent_and_stale_files_boot_normally() {
        assert_eq!(
            decide(&gateway_api_discovery::Resolution::Absent),
            Relaunch::Boot
        );
        for reason in [
            gateway_api_discovery::StaleReason::Invalid,
            gateway_api_discovery::StaleReason::ProcessDead,
            gateway_api_discovery::StaleReason::ImageMismatch,
            gateway_api_discovery::StaleReason::HealthFailed,
            gateway_api_discovery::StaleReason::KeyRejected,
        ] {
            assert_eq!(
                decide(&gateway_api_discovery::Resolution::Stale(reason)),
                Relaunch::Boot,
    ...
    fn a_stale_file_boots_and_is_cleaned() {
        ...
        drop(child);
        let file = gateway_api_discovery::GatewayDiscoveryFile { pid, ..live_file() };
        ...
        assert!(
            !gateway_api_discovery::gateway_discovery_file_path(temp.path()).exists(),
    ...
    fn a_connection_resolution_failure_stops_startup_with_its_source() {
        let temp = tempfile::TempDir::new().expect("tempdir");
        let connection_path = gateway_api_discovery::gateway_discovery_file_path(temp.path());
        ...
        assert!(
            matches!(
                error,
                GatewayStartupError::Resolve(gateway_api_discovery::SidecarError::Read { .. })
            ),
