...
pub fn diagnostics_json(explicit_config: Option<PathBuf>) -> String {
    let run_dir = gateway_api_discovery::default_run_dir();
    ...
    let running = run_dir
        .as_deref()
        .is_some_and(gateway_api_discovery::is_running);
    render(
        state_dir.as_deref(),
...
fn render(
    ...
    let discovery_file = run_dir.map(gateway_api_discovery::gateway_discovery_file_path);
