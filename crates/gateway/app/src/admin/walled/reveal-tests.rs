...
async fn serve_reveal(config: Config, paths: AdminPaths) -> (SocketAddr, Arc<RecordingLauncher>) {
...
async fn reveal_rejects_a_path_outside_the_safe_roots() {
    ...
    for outsider in outsiders {
        let response = post_reveal(addr, Some("test-token"), &outsider.display().to_string()).await;
        assert_eq!(
            response.status(),
...
async fn reveal_refuses_a_traversal_that_resolves_outside() {
    ...
    let response = post_reveal(addr, Some("test-token"), &traversal.display().to_string()).await;
