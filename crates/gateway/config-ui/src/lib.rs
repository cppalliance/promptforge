...
//! routes are served without bearer auth - the SPA shell holds no
//! secrets - but they are wrapped in [`require_loopback`], the single
//! shared loopback check the gateway also applies to its admin config
//! endpoints. The check itself is defined in the always-on
//! `shared-loopback` crate (re-exported here), so headless gateway
//! builds have the same wall without compiling this crate's asset
//! machinery.
//!
