...
- The gateway never hosts or embeds the workshop: the desktop shell spawns `workshop-server` in-process and attaches over HTTP, and no Gateway crate depends on `workshop-server`. A boot config carrying a `[workshop]` section must keep parsing - startup logs a deprecation warning naming the inert `bind`/`open_browser` fields; never fail or silently ignore it.
