...
For each top-level section, execution creates a fresh `SectionVm`, loads shared bytecode, injects `args`, `sys`, a new `var`, the shared `store`, replay-backed `tools`, and nil `reply`, then runs the preamble. Shared functions can read host globals after injection because they retain the same environment. H1 declaration calls replay exactly from immutable bindings and perform no embeddings.
