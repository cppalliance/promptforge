...
10. **The store and reply are the intentional mutable state across sections.** One caller-owned `StoreRef` exposes a run-scoped virtual filesystem: write, append, read_lines (numbered), read (verbatim), str_replace, delete, glob, and `exists(path)` (boolean, no error on missing). Model-facing re-injection wraps a verbatim read in an untrusted guard envelope via the `untrusted(s)` Lua global. The previous section's model reply carries forward as Lua `reply` (nil in section 1) and as `{{ reply }}` in prose. Section VMs and conversations are discarded.
