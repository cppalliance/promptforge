...
Those H1 blocks run live exactly once, in source order. `tools.need`, `models.need`, and `models.default` resolve when reached, H1 may call `models.infer`, and H1 `var` and store effects become run state. One exact `lua shared` fence may appear anywhere in H1. The parser compiles that separate library chunk as `Prompt.replay`; each section VM loads it before any host APIs or frozen capability objects are installed.
...
11. **Tool, Model, and Section are first-class Lua objects.**
    ...
    - `models.need` / `models.default` return a Model userdata: `.name`, `.model_id`, `.description`, `.context`, `.thinking`, `.temperature`, `.max_tokens`, `.dialect`.
