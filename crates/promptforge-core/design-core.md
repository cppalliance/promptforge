...
Those H1 blocks run live exactly once, in source order. `tools.need`, `models.need`, and `models.default` resolve when reached, H1 may call `model:infer`, and H1 `var` and store effects become run state. One exact `lua shared` fence may appear anywhere in H1. The parser compiles that separate library chunk as `Prompt.replay`; each section VM loads it before any host APIs or frozen capability objects are installed.
...
12. **`model:infer(prompt, opts?)` is explicit inference from Lua.** Available during live H1 and section execution via `block_in_place` + `Handle::block_on` (same pattern as fanout). It snapshot-reads the current tool bag, runs the tool loop on the handle's specific model, returns text, sets the `reply` global, increments `tools.calls`, and publishes `sys.reply_finish_reason`. Multiple infers: last completed inference wins for `reply`. The implicit prose path stays executor-driven and does not call `infer`. Two companions cover the cases `model:infer` does not: `models.get(alias)` returns the handle for a pre-declared model without changing the section's model selection, and `models.infer(prompt)` runs one direct, tool-free gateway round on a fresh conversation with the section's current model (`models.use` selection, else the `models.default` baseline), leaving `reply` and `sys.reply_finish_reason` untouched.

13. **`model:infer(y)` in Lua, prose blocks, and `fanout(worker, items)` are all model interactions that read live tool and model state at their point in the walk.** There is no scope open/close lifecycle - declarations are live until the VM is torn down. Tool schemas are rebuilt at each model interaction point. `models.default` in H1 sets the prompt-wide baseline model; `models.use` in an H2 section overrides it for that section (at most once per section). A prompt can carry both.

14. **`ToolBag` caches schemas behind a generation counter.** `tools.add` and `tools.add_local` bump `ToolRuntime::generation`. `model:infer` rebuilds schemas/dispatch only on generation mismatch. Counts persist across infer and prose calls within the section; newly added tools seed at 0. The implicit prose path rebuilds effective schemas and dispatch on every prose block, so `tools.add` and `tools.add_local` calls between prose blocks take effect at the next block.

15. **`tools.add_local(alias, description, params, handler)` declares a Lua-backed tool.** H2-only. The params table maps each parameter name to a bare type string or a `{type, description}` array; supported types are `string`, `integer`, `number`, and `boolean`, and every declared parameter is required. The handler runs synchronously in the declaring section's VM with full access to its globals and store, receives the arguments as a Lua table, and returns a string; Lua errors surface as tool-call failures. Handlers may call `execute`, `fanout`, and `model:infer`; `jump` is nilled for the duration of the call and restored after. Local tool output is trusted - no nonce envelope - because the prompt author wrote the handler. Local tools join the effective schema set at the next prose block or `model:infer`, and dispatch through a sentinel `ToolId("local", alias)` ahead of the registry.
