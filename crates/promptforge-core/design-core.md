...
Those H1 blocks run live exactly once, in source order. `tools.need`, `models.need`, and `models.only` resolve when reached, H1 may call `model:infer`, and H1 `var` and store effects become run state. One exact `lua shared` fence may appear anywhere in H1. The parser compiles that separate library chunk as `Prompt.replay`; each section VM loads it before any host APIs or frozen capability objects are installed.
...
11. **Tool, Model, and Section are first-class Lua objects.**
    ...
    - `models.need` / `models.only` return a Model userdata: `.name`, `.model_id`, `.description`, `.context`, `.thinking`, `.temperature`, `.max_tokens`, `.dialect`.
...
13. **`model:infer(y)` in Lua, prose blocks, and `fanout(worker, items)` are all model interactions that read live tool and model state at their point in the walk.** There is no scope open/close lifecycle - declarations are live until the VM is torn down. Tool schemas are snapshot-read at each model interaction point via `ToolBag` generation caching. `models.only` in H1 forecloses `models.use` in H2 sections (and vice versa - the choice is one model for the entire prompt or per-section selection, never mixed).

14. **`ToolBag` caches schemas behind a generation counter.** `tools.add` bumps `ToolRuntime::generation`. `model:infer` rebuilds schemas/dispatch only on generation mismatch. Counts persist across infer and prose calls within the section; newly added tools seed at 0. The implicit prose path still uses `prepare_effective_scope`.

15. **`execute(target, input?)` runs a section as a subroutine.** Fresh VM, fresh conversation, a fresh copy of the captured H1 `var`, same store/observer/execution id/gateway/registry. Recursion capped at 8. Returns the section's reply string. Target is `"## Name"` or a Section object.

16. **`jump(target)` transfers control.** The current section stops (no further blocks). Conversation and cross-section `reply` clear. The named top-level H2 runs next. No return to the caller. Unknown target is a hard error.

17. **Prompts resolve semantic needs live under local aliases.** H1 `tools.need(alias, description)` and `models.need(alias, description, opts?)` use case-sensitive aliases matching `[A-Za-z][A-Za-z0-9_-]{0,63}`. Each call immediately asks the prepared picker and returns a frozen Tool or Model object. Optional model `opts`: `context`, `thinking`, `temperature`, `max_tokens`. A need exposes nothing and selects no model. H2 `tools.add` makes tools available to the model; H1 `tools.always` is for every model-facing section. H2 `models.use` selects at most one captured model; H1 `models.only` is the prompt-wide default. Non-empty model-facing prose without either fails with `Error::ModelRequired`.

18. **Live H1 resolution captures one-to-one maps during execution.** Conditional needs resolve only when their branch runs. Duplicate live IDs, duplicate aliases, two aliases selecting one tool ID, and selected IDs absent from the registry fail at the resolving call. Rust installs the captured handles directly into every section VM. Effective-scope near-duplicates fail before a model sees tools.

19. **Per-VM `tools.calls` counts measure model behaviour.** Seeded at 0 for every in-scope alias. Incremented when dispatch is attempted. Unknown keys are hard errors. Out-of-scope model tool calls are `Error::OutOfScopeToolCall`. Counts are not rolled up across fanout arms.

20. **`sys` is sealed runtime metadata.** Always after inject: `sys.when`, `sys.now`, `sys.id`, `sys.section_name`, `sys.execution`, `sys.section_count`. After model resolution at first model interaction: `sys.model` (bound catalog id). After prose or infer completes: `sys.reply_finish_reason` (`"stop"`, `"length"`, `"tool_calls"`, or nil). Unknown field reads and any writes raise. Prose substitution also resolves `{{ args }}`, `{{ reply }}`, `{{ var.x }}`, and the `sys.*` paths above.

21. **Fanout returns structured arm results.** `fanout("### Worker", "### List")` still runs list-only H3 siblings concurrently on a `JoinSet`. Each result object has `.text`, `.ok`, `.item`, `.exhausted`. Soft-degraded exhaustion sets `.ok = false` and `.exhausted = true` with stub text that retains failure metadata. `__tostring` returns `.text` so existing `tostring` / coercing `table.concat` callers keep working. Fatal arm errors abort siblings. Children never execute by fall-through.

22. **The observer reports facts; raw payloads use opt-in `DebugCapture`.** Always-present `&dyn Observer`; silence is `NullObserver`. Fixed details are payload-free. Constrained phase-local `log(message)` is the sole author-controlled `Lua: <message>` exception (UTF-8, ≤256 chars, no newlines/controls). `RunOptions::debug` receives owned request/response JSON without widening the observer.

23. **Untrusted and executable content stay visibly separated.** Restricted Lua VM, instruction budget, no `print`/loaders/reflection. Untrusted tool results are nonce-framed before model history.

24. **Tool dialects own prepare, parse, and history echo.** `openai` and `gemma3_tool_code` ship. Author prompts never name a dialect. Empty model product is always `Error::EmptyModelReply`.

25. **Complete prompt fixtures live at the public crate boundary** in unpublished `promptforge-core-tests`. Ordinary tests stay offline; opt-in real-model scenarios go through a temporary gateway sidecar. Interactive authoring is `promptforge-dev` against an already-running gateway.
...
5. Before the first prose, snapshot tool bindings, resolve model, seed `tools.calls`, enrich `sys.model`, and prepare effective tool schemas.
