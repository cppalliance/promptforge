...
`StoreRef` is a cheaply cloneable handle over `Arc<Mutex<Box<dyn Store + Send + Sync>>>`, so the same files are reachable from the synchronous Lua VM and from an asynchronous tool. `StoreRef::memory()` builds one over `MemStore`, the in-memory backend, and `Store` is the backend contract a filesystem or network backend would implement.
