...
//!
//! These live in `promptforge-api` (not in `promptforge-lua`) because the
...
fn a_traceback_through_a_shim_shows_unmapped_impl_frames() {
    ...
    assert!(
        raw.contains("crates/promptforge-api/src/lua/__impl_coro.lua:"),
    ...
    assert!(
        mapped.contains("crates/promptforge-api/src/lua/__impl_coro.lua:"),
...
fn at_named_chunk_errors_render_verbatim_through_resume() {
    ...
    let program = LuaProgram::compile_internal(
        "local x = nil\nreturn x.field",
        "@crates/promptforge-api/src/lua/__impl_probe.lua",
    )
    ...
    let raw = error.to_string();
    assert!(
        raw.contains("crates/promptforge-api/src/lua/__impl_probe.lua:2:"),
