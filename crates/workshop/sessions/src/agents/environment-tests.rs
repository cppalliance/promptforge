//! Tests for the session registry, the launch-time current-model
//! resolution, and the end-to-end chat session over the activated web
//! capability.
...
#[test]
fn the_session_registry_builds_only_from_usable_gateway_settings() {
    assert!(
        session_registry("http://127.0.0.1:1", "k").is_some(),
        "a well-shaped root and key build the registry"
    );
    assert!(
        session_registry("http://127.0.0.1:1", "").is_none(),
        "an empty key cannot authenticate the search proxy"
    );
    assert!(session_registry("not a url", "k").is_none());
}
