//! The session's input broker: suspends an agent program until its
//! operator answers, guarded so a dying wait is an outcome, never
//! silence.
...
/// registry and pushes `input_cancelled` for its token. The broker future
