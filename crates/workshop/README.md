...
The gateway client: bearer-auth HTTP, endpoint binding and discovery, heartbeat, the progress subscriber (which decodes the gateway's `Progress` snapshots and drives the status bar's busy frames), and the run event log. The server's subsystems reach the gateway through it. Depends on workshop-protocol, workshop-registry, workshop-support, promptforge, gateway-api-types, and gateway-api-discovery.
...
The wire protocol: every JSON frame over the workshop sockets, typed in one place, zero I/O. Every subsystem and the SPA share it as the frame contract. Depends on promptforge.
