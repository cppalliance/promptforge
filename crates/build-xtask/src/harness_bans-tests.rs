...
//! this workspace's `crates/harness/` container and its `crates/harness-api/`
//! public crate.
...
/// A fake workspace root; `container` and `public_crate` name its harness
...
fn public_crate(root: &Path) -> std::path::PathBuf {
...
#[test]
fn an_absent_container_and_public_crate_are_vacuously_clean() {
    let root = fake_root();
    let violations = harness_clippy_bans(&container(root.path()), &public_crate(root.path()));
...
fn a_complete_clippy_toml_in_either_entry_form_passes() {
    ...
    write_crate(&public_crate(root.path()), Some(COMPLETE));
    let violations = harness_clippy_bans(&container(root.path()), &public_crate(root.path()));
    assert!(violations.is_empty(), "{violations:?}");
}

#[test]
fn the_public_crate_is_held_to_the_same_bans() {
    let root = fake_root();
    write_crate(&public_crate(root.path()), None);
    let violations = harness_clippy_bans(&container(root.path()), &public_crate(root.path()));
    assert_eq!(violations.len(), 1, "{violations:?}");
    assert!(
        violations[0].contains("harness-api"),
        "the public crate is named: {violations:?}"
    );
}
...
#[test]
fn the_ban_check_covers_the_eight_container_crates_and_the_public_crate() {
