// The persistent workbench socket: one WebSocket to /ws carries every
...
export class WorkbenchSocket {
  ...
  async streamChat(
    ...
    if (!socket || socket.readyState !== WebSocket.OPEN) {
      throw new Error("the workbench socket is not open");
    }
  ...
  private ensureOpen(): Promise<void> {
    ...
    entry.promise = new Promise<void>((resolve, reject) => {
      ...
      socket.onerror = () => {
        ...
        if (this.opening === entry) this.opening = null;
        reject(new Error("the workbench socket failed to open"));
      };
    });
    this.opening = entry;
    ...
    for (const id of [...this.pending.keys()]) {
      this.settle(id, (chat) => {
        if (chat.started) {
          ...
          chat.reject(new Error("the workbench socket closed before the reply completed"));
        }
      });
    }
  }
