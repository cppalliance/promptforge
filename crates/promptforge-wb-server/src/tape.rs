...
//! `tape.path` in `workbench.toml`, one JSON object per line. The tape is
...
pub struct TapeEvent {
    ...
    pub model: String,
    /// The request body as the workbench received it.
...
impl Tape {
    ...
    /// let tape = promptforge_wb_server::Tape::open(&dir.path().join("tape.jsonl"))?;
