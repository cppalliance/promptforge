...
-- tasks.when_any(set) -> Task, ok, result: park until the first member of
-- `set` ends (or return at once when one already has) and return which
-- one, whether it succeeded, and its final text or error value. The one
-- scheduler wait primitive; the error value is returned, never raised, so
-- the caller decides. A member the caller does not own raises
-- task_not_owned; a member already delivered raises task_consumed.
local function tasks_when_any(set)
  local ids = task_set(set, "tasks.when_any")
  local ok, task, task_ok, result = yield({ op = "when_any", tasks = ids })
  if not ok then fail(task) end
  return { task = task }, task_ok, result
end

-- tasks.when_all(set) -> results: Lua over when_any. Waits for every
-- member and returns `{ task, ok, result }` per member in input order;
-- each entry is itself a Task handle. It never raises because a member
-- failed - the failed member's entry carries `ok = false` and the error
-- value - so no caller is forced into a cancel-or-leak choice for the
-- members still running. A member named twice is waited on once and
-- fills every position it was named at, so the result sequence has no
-- holes and `#results` is the input's length.
local function tasks_when_all(set)
  ...
  while #remaining > 0 do
    local task, ok, result = tasks_when_any(remaining)
    for index, id in ipairs(ids) do
      if id == task.task then
        results[index] = { task = id, ok = ok, result = result }
      end
    end
    local rest = {}
    for _, id in ipairs(remaining) do
      if id ~= task.task then rest[#rest + 1] = id end
    end
    remaining = rest
  end
  return results
