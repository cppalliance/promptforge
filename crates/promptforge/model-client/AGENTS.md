...
- No HTTP. The crate never opens a connection, names an HTTP client, or reads the environment. `reqwest` and `url` are not dependencies; a change that needs them belongs in `harness-models`, which reaches this vocabulary through the `promptforge-api-runtime` door.
