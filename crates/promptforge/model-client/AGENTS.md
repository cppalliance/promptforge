...
- `CompletionError` is the failure a round reports. Its `#[doc(hidden)]` internal type (`Error`, `Timeout`) is public only so the runtime maps it verbatim and a transport can construct it; it is not host API.
