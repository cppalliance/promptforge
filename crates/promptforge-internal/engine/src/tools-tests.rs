//! Regression coverage for the `promptforge_engine::tools` compatibility
//! re-exports: the contract vocabulary sits in `promptforge-types`'s
...
fn reexported_types_are_the_contract_types() {
    ...
    // same type.
    fn takes_contract_id(id: &promptforge_types::tools::ToolId) -> &str {
        id.name()
    }
    fn takes_contract_catalog(catalog: &promptforge_types::tools::ToolCatalog) -> usize {
        catalog.tools().len()
    }
