...
//! the engine alone forks it for concurrent arms.

use crate::{Access, Origin, VfsError};

/// Returns the capability for a new concurrent thread of execution.
///
/// The child gets a fresh [`ExecId`](crate::ExecId), and `parent`'s claims
/// are deleted from the tables: they predate the child by construction, so
/// a retired claim can never conflict again. The spawn IS the happens-before
/// edge - no fence call, no epochs. `origin` labels the child's operation
/// events, as in [`VfsRef::acquire`](crate::VfsRef::acquire).
///
/// # Errors
