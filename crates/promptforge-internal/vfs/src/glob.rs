...
//!
//! The backend validates a caller-supplied glob against one grammar and
...
/// Validates the glob grammar, rejecting unsupported forms.
///
/// The grammar is: literal bytes, `*` (within a segment), and `**`
/// occupying a whole segment (`**`, `**/...`, `.../**`, `.../**/...`).
/// There is no escape syntax, so a backslash is unsupported and runs of
/// three or more `*` are rejected rather than silently reinterpreted.
...
/// Compiles an already-grammar-validated pattern into reusable
/// [`GlobToken`]s. Callers that match one pattern against many paths
/// compile once and reuse the tokens via [`matches_tokens`], so the
/// per-path tokenization cost is not repeated for every key.
...
/// Matches `text` against a glob `pattern` where `*` stays within a
/// segment and `**` spans `/`. Bounded iterative dynamic programming over
/// reachable text positions, with no recursion and no suffix backtracking:
/// `O(tokens * text_len)` time and `O(text_len)` space. Compiles and
...
/// Matches `text` against already-compiled glob `tokens` (see
/// [`compile_glob`]). Split from the one-shot matcher so one compiled
/// pattern can be reused across many paths without re-tokenizing per path.
pub(crate) fn matches_tokens(tokens: &[GlobToken], text: &[u8]) -> bool {
    let len = text.len();
    // `reachable[j]` is true when some prefix of the pattern consumed so
    // far matches exactly `text[..j]`.
...
mod tests {
    ...
    fn compiled_and_one_shot_match_expected_results_across_many_paths() {
        // A pattern compiled once and reused across many keys must produce
        // the pinned result for every key. The one-shot matcher is checked
        // against the same independent expectations.
        let paths = [
            "a.txt",
            "src/a.rs",
            "src/b.rs",
