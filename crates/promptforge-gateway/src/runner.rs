...
fn load_startup_with_environment(
    ...
    let selection = promptforge_gateway_config::ProfileSelection::new(
    ...
    let active = config
        ...
        .transpose()
        .map_err(|error| {
            StartupError::config(promptforge_gateway_config::ConfigError::validation(
                error.to_string(),
            ))
        })?;
    Ok((
...
"#;
    ...
    fn fixture(state: &str) -> (tempfile::TempDir, PathBuf) {
        ...
        std::fs::write(
            promptforge_gateway_config::profile_state_path(&path),
