...
mod tests {
    ...
    /// Serves `build_router` over a state assembled from `config` with an
    /// empty local runtime: no `llama-server` children start, but the
    /// retained config still carries the `[[local_model]]` entries the
    /// orphan scan diffs against.
