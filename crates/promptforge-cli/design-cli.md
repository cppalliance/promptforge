...
`promptforge run <file.md> [input]` reads a prompt from the filesystem, executes its sections in this process, and prints the run's returned value on stdout. There is no catalog and no configuration file: the binary links the executor rather than calling one. Only a run that makes no model call is self-contained that way; the moment a section carries prose, the executor builds a gateway client from the environment, so a model turn needs `PROMPTFORGE_TOKEN` and a reachable gateway.
...
Two facts bound what a run can do. The store backing the run is in memory, so a prompt's filed state lives exactly as long as the process, and the only tools that resolve are `web_fetch`, which runs locally, and `web_search`, which proxies through the gateway and therefore needs `PROMPTFORGE_BASE_URL` and `PROMPTFORGE_TOKEN` in the environment. A prompt naming any other tool is refused before the run starts.
...
9. **Tools resolve by canonical name at startup, before the run begins.** The prompt's frontmatter lists the names it needs; the binary maps each to an instance and fails the whole invocation if any name has no implementation here. Resolving up front rather than at first call means a prompt that would have failed twenty minutes into a run fails immediately, and the error names the tool. Two names resolve: `web_fetch` locally, always, and `web_search` through the gateway.

10. **`web_search` requires the gateway, and its absence is an error rather than a degraded run.** The instance is built from `PROMPTFORGE_BASE_URL` and `PROMPTFORGE_TOKEN`, and when either is missing the binary refuses and names both variables. Silently dropping the tool would leave a model to discover mid-run that it cannot search, and a report written without the searches its prompt asked for is worse than no report. The same two variables also carry the model client every prose section builds, so they are not the search tool's alone; `web_search` is only the one case whose absence is refused up front rather than at the moment of use.
...
13. **Progress is discarded.** The observer is the null one, so the section-by-section event stream the executor emits reaches nothing. The binary prints one thing when the run is over, which is the value; a spinner or a live status line would be output that a pipeline has to be taught to ignore. A long run therefore looks silent, which is accepted here and is the reason a rendering client is worth building elsewhere.
