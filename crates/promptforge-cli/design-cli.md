...
Two facts bound what a run can do. The store backing the run is in memory, so a prompt's filed state lives exactly as long as the process, and the execution tool pool is temporarily empty. The CLI does not yet bind H1 capability declarations to live tools, so no tool schema is advertised to the model and a reached legacy `tools.add(...)` call cannot resolve its name from the pool.
...
9. **Concrete tool selection is temporarily absent.** Prompt frontmatter no longer carries concrete tool names. The binary currently calls its concrete selector with an empty request and passes the resulting empty tool pool to core. This is transitional: semantic H1 capability binding and CLI live-registry integration are planned, but the CLI does not yet run that binding or assemble the selected live tools.

10. **Tool-specific gateway validation is dormant while the pool is empty.** The retained concrete selector would require `PROMPTFORGE_BASE_URL` and `PROMPTFORGE_TOKEN` if asked to construct `web_search`, but its current empty request constructs nothing and performs no such up-front check. A prose section can still require gateway configuration for its model turn; a Lua-only run that returns before a model turn does not. The planned semantic binding path will make live-tool construction and its validation relevant again, but that integration has not shipped.
