...
10. **Unavailable gateway tools are omitted, not left as dead descriptors.** `web_fetch` needs no credential and is always included. `web_search` needs the gateway URL and bearer, so without both `PROMPTFORGE_GATEWAY_URL` and `PROMPTFORGE_GATEWAY_KEY` neither its live instance nor its picker descriptor exists and a matching need produces the ordinary absent-capability binding error. When the key is present without a URL the CLI fails before binding rather than inventing a gateway location. This tests availability at construction rather than advertising a tool whose first call is guaranteed to fail authentication.

11. **The environment carries the gateway's location and credential, and the command line does not.** There is no `--url`, no `--token-file`, and no `--model`, so a key never appears in `argv` where `ps` and shell history can see it. Two names are read for model turns and gateway-backed tools: `PROMPTFORGE_GATEWAY_URL`, the gateway API root, and `PROMPTFORGE_GATEWAY_KEY`, the bearer, both required on the first model turn through `GatewayClient::from_env`. The key alone is not enough for `web_search`; the URL must be set too. Model selection comes from prompt bindings, not from the environment.
