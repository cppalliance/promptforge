...
- Unsafe is confined to the `src/sys/` process-image shims, where each module is marked `#[expect(unsafe_code)]` with its reason and every block has a `// SAFETY:` comment; the crate-level lint is `deny` everywhere else.
