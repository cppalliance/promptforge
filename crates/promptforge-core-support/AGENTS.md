...
wrapping (`untrusted`), cooperative cancellation (`cancel`), report-only
run observation (`observe`), and the canonical metrics and runtime-event
vocabulary with its read-side log interface (`events`).
