...
fn record(task_id: u64, task_seq: u32, kind: RecordKind) -> Record {
    Record {
        task_id,
        task_seq,
        kind,
...
/// Two tasks interleaved in loop order, each task's events arriving out
/// of `task_seq` order, with an effect and its answer mixed in so the
/// event-only readers have something to exclude. Returns the run.
async fn interleaved_run(log: &mut RunLog) -> Result<RunId, LogError> {
    let run = log.begin_run(meta()).await?;
    let appended = [
        (0, 0, RecordKind::Event),
        (1, 2, RecordKind::Event),
...
async fn events_for_task_returns_one_task_in_task_seq_order_when_tasks_interleave() {
    ...
    let task_one = log.events_for_task(run, 1, None).await.unwrap();
    assert_eq!(task_seqs(&task_one), expected([0, 1, 2]));
    for payload in &task_one {
        assert_eq!(payload["task"], json!(1));
    }
}
...
async fn events_for_task_last_n_keeps_the_final_n_by_task_seq() {
    ...
    let last_two = log.events_for_task(run, 1, Some(2)).await.unwrap();
    ...
    let more_than_exist = log.events_for_task(run, 1, Some(10)).await.unwrap();
    assert_eq!(task_seqs(&more_than_exist), expected([0, 1, 2]));

    let none = log.events_for_task(run, 1, Some(0)).await.unwrap();
...
async fn events_for_task_is_empty_for_a_task_that_never_logged() {
    let mut log = RunLog::in_memory().await.unwrap();
    let run = interleaved_run(&mut log).await.unwrap();
    assert!(log.events_for_task(run, 9, None).await.unwrap().is_empty());
}
...
async fn transcript_returns_every_event_in_seq_order_and_nothing_else() {
    ...
    let provenance: Vec<(u64, u32)> = transcript
        .iter()
        .map(|stored| (stored.record.task_id, stored.record.task_seq))
        .collect();
    assert_eq!(provenance, [(0, 0), (1, 2), (1, 0), (0, 3), (1, 1), (0, 4)]);
    assert!(
        transcript
            .iter()
