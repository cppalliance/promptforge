...
- Shared vocabulary comes from below: wire types, `Upstream`, and `http_util` from `gateway-protocol`; `Model`, `Endpoint`, and the dominion queues from `gateway-routing`. This crate never names gateway concepts (`GatewayError`, `Routing`, profile switching).
