...
- Absent, not stubbed: `call`, `fanout`, and `jump` do not exist in an agent VM. An agent touching them fails as an undefined global, exactly as a document prompt touching `models.chat` does. No courtesy stubs, no typed errors for absent calls.
