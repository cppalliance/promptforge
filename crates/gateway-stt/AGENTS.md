...
- Runtime ownership only. Whisper inference primitives stay in
  `gateway-transcribe`; artifact download and verification stay in
  `gateway-local::artifacts::ArtifactStore`.
...
- The whisper.cpp runtime is provisioned through `ArtifactStore` and handed
  to `gateway-transcribe` as a path. Native backends are never Cargo
  features.
