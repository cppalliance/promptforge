...
12. **Three flat parameters carry file content in and out of a run.** `run_prompt` accepts `input_file`, `input_text`, and `output_file`, all optional strings alongside `prompt` and `args`. Flat strings rather than structured objects because every MCP tool parameter a model fills is a string at heart, and a nested schema for one path adds parsing the model rarely gets right on the first call.

    `input_file` and `input_text` are mutually exclusive: supplying both is `-32602`, since the ambiguity is the caller's own bug rather than something the model can self-correct. Validation rejects two further mismatches as `isError` results the model can fix: input supplied when the prompt declares no `input` (the caller is feeding a prompt that does not eat), and a prompt declaring `input` when the call supplies none (the prompt expects content the caller forgot). Both carry the prompt's declaration so the model can see what it missed.

    The gateway execution flow is: read `input_file` from disk (or accept `input_text` verbatim), seed a `MemStore` with the content at the declared input path, run the prompt sandboxed, extract the declared output path from the store, then either write it to `output_file` on disk or return it inline in the result. The prompt itself never touches the filesystem - it reads and writes store paths that happen to have been seeded from, or flushed to, disk by the gateway layer around it.

    The safety model follows from this layering. The orchestrator (the calling model or its harness) owns the real paths it passes. The gateway trusts the caller's paths and performs the I/O. The prompt is sandboxed: it sees store paths, never disk paths, and cannot reach outside the store. A prompt that declares `output: report.md` has no way to know whether the gateway writes that to `/tmp/report.md` or returns it inline - and that ignorance is the isolation boundary.

    `list_prompts` includes `input` and `output` metadata per prompt (path and description from each `FileDecl`) so a caller can discover file expectations before calling `run_prompt`.

13. **Progress is first-class, and the runtime exposes one report-only observer seam to carry it.** Without it every run is one silent multi-minute call. `promptforge-core` calls `Observer::observe(execution, section, detail)` with borrowed strings. Section names and ordinary details are payload-free static labels; the sole dynamic exception is a constrained author checkpoint in the exact form `Lua: <message>`. The server recognizes a small exact detail vocabulary for `notifications/progress`, which a client renders as a caption changing in place. It logs the Lua exception at debug level but never turns it into progress. The server reuses its existing run id as `execution` across per-run parsing, live H1, and section execution. Unknown details are tolerated and reports never steer execution. Progress buys visibility only, never time: choice 5, not this one, is what keeps a long run alive.

14. **`need_prompt` hands back three candidates and never runs one.** A plain-English capability is ranked against the catalog and up to three prompts come back, best first, for the calling model to choose among with the whole conversation in front of it. It then calls `run_prompt` with a name it was handed rather than one it guessed, which closes the gap choice 10 can only mitigate. It does not contradict choice 1, because the intent still came from a person naming what they want: the caller who was told to run "the prompt that builds a stakeholder report" without being told it is called `staker`. A retriever that bound instead of suggesting would spend minutes of gateway time on the wrong artifact, and the caller could not tell that it had.
...
| `run_prompt` | `prompt`, optional `args`, optional `input_file`, optional `input_text`, optional `output_file` | always |
| `need_prompt` | `capability` | with the `picker` feature |
| `check_run` | `run_id` | always |

Every property of every schema is a string, because a prompt name, a raw argument string, a run id, a capability, and a file path are all strings and no other property type is offered. `run_prompt` requires `prompt` alone, so a missing `args` is the empty string and missing file parameters mean the prompt runs without file I/O.
...
The gateway URL and key live in the file and the server constructs a gateway client explicitly. This is forced rather than chosen: setting a process environment variable is `unsafe` under edition 2024 and this workspace forbids unsafe, so the runtime's environment-based client cannot be driven from a config file. The runtime therefore takes an optional client, and `None` keeps the environment path the CLI uses. Model selection comes from models resolved by live H1, not from MCP config.
...
api_key = "${PROMPTFORGE_MCP_SERVER_API_KEY}"   # required to bind HTTP, unread on stdio
...
Two details of the interpolation are load-bearing. `${VAR}` is expanded over the *parsed* document rather than the raw text, which is what attributes an unset variable to the field that carried it; that is the only way `[server].api_key` alone can survive one while every other field still fails the load. And an unset variable anywhere else fails the load, so the server never starts with a blank credential.

The one source of values outside the file is the name-matched env file: `prompts.env` beside `prompts.toml`, parsed at load into an in-memory map that supplies `${VAR}` defaults. The process environment wins over the file, which is the precedence the gateway's dotenvy loading gets from its no-override semantics, so an operator can still override a file value for one launch. A missing env file is skipped silently and a malformed one is ignored with a warning, so the file can never fail a load the TOML would have passed. The values live in a map threaded into interpolation rather than in the process environment for one reason: setting an environment variable is `unsafe` under edition 2024 and this crate forbids unsafe, and the map reproduces the gateway's observable behavior - lookup order included - without it. The map is rebuilt on every load, so a reload that re-reads `prompts.toml` also re-reads the env file; a save of the env file alone does not trigger one, because the watcher answers to the configuration file and the prompts directory.
...
Each `run_prompt` call clones the parsed prompt snapshot and admits it as a run. The executor runs H1 live exactly once. A capability need performs synchronous embedding work through the prepared picker at the point where H1 reaches the call. Conditional branches therefore resolve only what they execute, and capability failures are ordinary execution failures. The same `McpObserver` instance spans live H1 and section execution, so report ordering remains one lifecycle.

The run task owns a `Prompt` alongside the shared picker, model catalog, and tool environment. Live H1 resolution captures frozen aliases to stable `ToolId` and `ModelId` values; section VMs receive those captured objects directly from Rust. The registry and picker remain fixed across catalog reloads because saving prompt text changes neither the configured gateway nor the host's available tool set.
...
Prompt retrieval is optional and on by default, but the ranking engine is no longer an optional dependency: live H1 capability resolution uses the same engine. `--no-default-features` therefore drops `need_prompt` alone and does not remove the roughly 67MB of compiled weights. Default on, because a server whose retrieval tool depends on remembering a flag is a server whose retrieval tool does not get used.

The execution picker is loaded before the Tokio runtime exists, since parsing the weights is seconds of blocking CPU and there is no worker yet to block. Its failure does fail boot because no prompt capability can resolve correctly without it. With retrieval compiled in, its separate prompt index is also prepared before Tokio; failure of that optional index is reported at error level while the process serves, because every prompt remains callable through the already-prepared execution picker.
...
The server creates its run id before work begins, reparses the validated catalog source snapshot with that id, and passes the same id through live H1 and `RunOptions`. It recognizes only exact constants from `promptforge_core::observe::detail`. `Run started` creates frame zero, `Section started` increments the progress counter, and `Model turn completed` increments the result's turn counter. Live-resolution and registry-validation labels, constrained `Lua: <message>` details, and any future core detail currently fall through the same unknown-detail path: they are logged at debug level with the execution id, emit no frame, change no counter, and cannot affect execution. Reports are never decisions, so dropping one cannot change a result, and that is what makes the queue behind the forwarding path allowed to lose frames.
