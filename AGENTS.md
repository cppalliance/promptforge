...
- No new `localStorage` stores. UI state has three homes by kind and scope: account preferences go to the machine-written TOML config, account ephemera to the JSON state file, and workspace-scoped state (preferences or ephemera that should travel with the `.pfwork` document) to the workspace file through the server. The five existing stores (`editor-settings-service`, `commands-history`, `recent-files-store`, `layout-persistence`, `zoom`) are legacy: maintain them in place, never add a sixth or join new state to them; they migrate in the settings project.
