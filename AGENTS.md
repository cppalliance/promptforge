...
- No `localStorage`. The SPA never reads or writes browser storage; every persisted value goes through the `ui-storage` adapter to the server. UI state has three homes by kind and scope: account preferences go to the machine-written TOML config, account ephemera to `ui-state.json` in the state directory (the `workshop-user-state` crate, `/user/state`), and workspace-scoped state (preferences or ephemera that should travel with the `.pfwork` document) to the workspace file through the server (`/workspace/file/state`). A new persisted value is a new allow-listed key in one of those buckets, added on the server first.
