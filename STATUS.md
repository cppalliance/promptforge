...
the gateway; the core-tests binary entry point is inert while its offline fixture harness is active.
...
- `promptforge-core` (lib) - the runtime. It parses and compiles required-H1 prompts with shared Lua and three-phase H2 sections, with no author-facing prompt revision field, binds exact capability declarations through `ToolPicker` to immutable one-to-one alias and stable-`ToolId` maps validated against the complete live registry, and retains selected descriptor diagnostics plus picker near-duplicate analysis. `execute::run` creates one isolated sendable `SectionVm` per section, replays frozen H1 declarations without embeddings, runs preamble, substituted prose and the complete model tool loop, binds `reply`, runs epilog, and tears down before fall-through. Explicit `tools.always` plus H2 `tools.add` form the only model-visible scope. Before a non-empty model turn, near-duplicates in that effective scope fail with rich diagnostics while similar tools in separate sections remain valid. Concrete descriptions and schemas are advertised under local aliases, and returned aliases dispatch through frozen `ToolId` values rather than transport wire names. Scope, model, tool, Lua, binding, registry, parse, and harness-store boundaries report fixed payload-free `(execution, section, detail)` records through `Observer::observe`; callers pass one stable id through parse, bind, and `RunOptions`, while `SectionVm` retains it for lifecycle and store reports. Every executable Lua VM removes `print`; H1 binding and replay, H2 preamble and epilog, and `run_chunk` instead install a phase-local borrowed `log(message)` that validates one control-free, single-line UTF-8 string of at most 256 characters and reports exact `Lua: <message>` details under the current id and H1 or H2. This is the sole author-controlled observer payload, and no callback or observer reference survives a phase or model await. Observer synchronization stays in each concrete recorder and no recorder lock crosses an await. `NullObserver` changes only visibility. Store persistence, scalar-return precedence, untrusted-result framing, and the `run_chunk` compatibility path remain intact. CLI and MCP live-registry integrations are complete. `design-core.md` is authoritative; `design-core-orig.md` remains unchanged history. 180 unit + 43 doc tests.
- `promptforge-core-tests` (bin) - the unpublished author-document fixture boundary. Three valid, three invalid, and three deterministic execution Markdown prompts are registered explicitly with `include_str!` and exercised through public parse, deterministic declaration-binding, and offline execution APIs. Valid cases assert frontmatter, title, shared Lua, section trees, and preamble/prose/epilog structure; invalid cases assert the public error variant and a stable message fragment, with fixture names carried into every failure. Tool-free execution cases bind shared declarations through a deterministic no-tools resolver, use the public parsed-`Prompt` compatibility seam, and assert exact constrained Lua checkpoint order, scalar preamble return before prose or epilog, and explicit store persistence across isolated fall-through sections. A mutex-backed observer records stable IDs, and two concurrent runs sharing it are partitioned exactly by execution ID. A separate smoke test discovers all four shipped repository prompts, rejects concrete tool names, and requires each to parse; the MCP server's owner tests retain complete-registry semantic binding coverage. The original untracked minimum prompt remains the canonical `valid/minimal.md` fixture with its content preserved. The harness directly depends only on core and performs no picker construction, model loading, production-tool provisioning, process launches, network requests, gateway calls, generation-model calls, or credential reads; a cold workspace build may still acquire assets for core's existing transitive picker dependency. The explicit binary entry point is inert until the separate real-model runner step. 7 unit tests.
