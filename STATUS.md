...
- `promptforge-core` (lib) - the runtime. `parser` (frontmatter without concrete tools, ignored pre-H1 Markdown, exactly one required H1, one optional exact immediately leading `lua prompt` shared library compiled at parse time, H1 description, recursive H2-H6 tree, and the existing leading H2 Lua fence); `client::GatewayClient` (OpenAI-shaped, aimed at the gateway, public `DEFAULT_MODEL`); `lua::LuaProgram` (retained source, one-time Lua 5.4 compilation, private process-local bytecode, location-bearing compile errors), sendable `lua::SectionVm` (one hardened isolated environment and instruction budget across optional shared load, delayed host injection, compiled preamble, `reply` binding, and compiled epilog, with scalar returns), deterministic `ToolResolver` plus immutable `ToolBindings` (exact H1 `tools.need` and `tools.always` binding, per-section replay without re-resolution, atomic H2 `tools.add` recording, one-way effective-scope closure, case-sensitive 64-character aliases, and phase restrictions), and the one-phase `lua::run_chunk` compatibility path; `subst::substitute` (`{{ args }}`, `{{ var.x }}`, `{{ sys.x }}`; pure lookup, no arithmetic - compute in Lua); `store` (run-scoped VFS, memory or files, exposed to Lua); `tools` (stable server-qualified `ToolId`, faithful description and parameter schema, transport-only `wire_name`, and ordered `ToolRegistry` lookup by identity); `observe`, the report-only seam (`Observer::observe(section, detail)`, stable payload-free parse, compilation, execution, and store details, `NullObserver`); and `execute::run(prompt, args, tools, store, opts)`, which walks top-level sections in file order, each in a fresh context - a Lua `return` finishes the run with no model call, otherwise the prose is substituted, takes one round trip, and control falls through. Off the end yields `default_return`, else the last reply, else a generic completion. The parser stores the shared program, but the executor does not yet run H1 declarations or shared programs and does not yet support epilogs or alias dispatch. Identity uniqueness and live-registry agreement remain later binding checks. `opts` is `RunOptions { observer, client }`; `client: None` builds from the environment (the CLI's path) and `Some(..)` is how a file-configured caller supplies one, since setting an env var is unsafe under edition 2024. Recording or dropping reports cannot change behavior. `design-core.md` is authoritative for built and clearly marked planned behavior; `design-core-orig.md` preserves the previous design unchanged.
