...
the gateway; the new core-tests binary is currently an inert fixture boundary.
...
- `promptforge-core-tests` (bin) - the unpublished author-document fixture boundary. Three valid and three invalid Markdown prompts are registered explicitly with `include_str!` and exercised in two table-driven offline unit tests through the public core parser API. Valid cases assert frontmatter, title, shared Lua, section trees, and preamble/prose/epilog structure; invalid cases assert the public error variant and a stable message fragment, with fixture names carried into every failure. The original untracked minimum prompt is now the canonical `valid/minimal.md` fixture with its content preserved. Ordinary tests perform no downloads, model calls, process launches, network access, or credential reads. The explicit binary entry point is inert until the separate real-model runner step.
