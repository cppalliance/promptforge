...
- `promptforge-cli` (bin `promptforge`) - `promptforge run <file.md> [input]`; refuses a file that declares no `promptforge:` version, builds the complete currently available live registry and a picker catalog derived from the same instances, binds H1 needs synchronously, and executes the resulting `BoundPrompt`. Local `web_fetch` is always available; gateway-backed `web_search` is omitted without `PROMPTFORGE_TOKEN`. One observer reference spans parsing, binding, and execution, with `NullObserver` installed by default. `design-cli.md` is its design document and is current. 4 unit tests.
...
- Observer `(section, detail)` pairs are authoritative trace records; any future model-generated label is optional off-path UI text and never replaces or steers the trace
