...
Demonstrates prompt-local model binding. H1 `models.need` resolves against the host's gateway catalog. A section that calls `models.use` runs every completion under that binding's frozen invocation; a section that omits it keeps the host default client model.
